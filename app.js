...
// Estado global
let cashFlows = [
  { period: 0, amount: -120000 },
  { period: 2, amount: 50000 },
  { period: 7, amount: 50000 },
  { period: 13, amount: 40000 }
];
let escenarios = [];
let alternativas = [];
let canvas, ctx;
  ...
  setupScroll: () => {
    if (!Utils.isMobile()) {
      ...
      document.body.addEventListener('wheel', (e) => {
        e.preventDefault();
        window.scrollBy({
          top: e.deltaY * CONSTANTS.SCROLL_SPEED,
          behavior: 'smooth'
        });
      }, { passive: false });
      ...
      document.body.addEventListener('touchmove', (e) => {
        ...
        window.scrollBy({
          top: -(currentY - startY) * CONSTANTS.SCROLL_SPEED,
          behavior: 'smooth'
        });
        startY = currentY;
      }, { passive: false });
...
const Calculations = {
  ...
  calcularBono: () => {
    ...
    let vp = 0, duracion = 0, convexidad = 0;
    for (let t = 1; t <= n; t++) {
  ...
  analizarSensibilidad: () => {
    ...
    window.sensitivityChart = new Chart(chartCtx, {
      ...
      options: {
        ...
        scales: {
          x: { title: { display: true, text: 'Variación (%)' } },
          y: { title: { display: true, text: 'Valor Presente Neto (VPN)' } }
        },
        plugins: {
...
const ScenarioManager = {
  agregarEscenario: () => {
    ...
    escenarios.push({ tipo, probabilidad, vpn });
